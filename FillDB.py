    ...
    if articles:
            ...
            for article in articles:
                ...
                if articleFromApi['mensaje'] == 'Articulo solicitado correctamente':
                    ...
                    update_query = "UPDATE articles SET categoria = %s,categoria_secundaria = %s ,categoria_terciaria = %s WHERE id = %s"
                    params = (articleFromApi['datos']['categoria_inicial_desc'],articleFromApi['datos']['categoria_secundaria_desc'],articleFromApi['datos']['categoria_terciaria_desc'], article['id'])

                    db.execute_query(update_query, params)
                    #db.update_record('precios_historicos', {'precio': articleFromApi['datos']['precio'], 'precio_promo': articleFromApi['datos']['precio_promo'], 'precio_anterior': articleFromApi['datos']['precio_anterior'], 'precio_unitario': articleFromApi['datos']['precio_unitario'], 'article_id': article['articleid'], 'business_id': article['businessid']})
                else:
